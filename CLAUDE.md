...
- Per-chord velocities stored in `ProgressionChord.midiVelocities` (parallel to `midiNotes`)
- Chord analysis only triggers on note-ON events (releasing notes does NOT create intermediate "stab" chords)
- Playback replays raw MIDI directly (not reconstructed from chords)
